...
Иногда кажется, что все под контролем и можно выдохнуть, 
но это ошибка — и я попробую рассказать почему.

## N
Мы все, ну покрайней мере такие мамонты как я, 
...
Появились поплуряные проекты в поддержку CSS.
Браузеры собревнуются в поддержке CSS.
