...
Я занимаюсь фронтенд разроботкой уже около 10 лет.
...
## История
Мы все помним каким был фронтенд 10 лет назад.
...
* бедное апи браузеров

И вообще тогда даже эра CSS только начиналась

## Каким был CSS
CSS почти не было, в пузырь доткомов и долгое время после сайты были вообще без стилей, все оформление описывалось прямо в HTML, ну вы в курсе, если и не застали этой поры. Тогда модным было понятие DHTML — я до сих пор не знаю что это такое :)

### Борьба за CSS
Потом пощла волна про разделение оформления и структуры документа.
Тогда в эту игру активно вступили бразуеры и начали соревноваться в поддержке CSS.
Появился ACID2, такой нашумевший проект как css zen-garden — вообщем хайпа в те времена было хватало и не сказать что было меньше чем сейчас.
...
Выходит jQuery и плагины растут как грибы после дождя, с поддержкой history API и локальных хранилищь начинют появляться Single Page Application.
...
Даже когда браузеры не успевают за спецификацией, появляются транспайлеры из новых версий языка в es5 — самый крутой и популярный Babel. Это сильно упрощает

## Отказ от jQuery
Все больше проектов откзываются от jq, основная цель которого была стандартизировать апи браузера, теперь это случилось и нужды в jQery все меньше и меньше.

## Бум MVC
Мы очень быстро пережили бум MVC фреймворков: knockout, backbone, angular..
Теперь MVC не очень модно и все любят паттерн Flux.

## Вымирает профессия верстальщика
Я — верстальщик. Я очень много занимался CSS и в мир JS пришел весьма поздно. Но как бы моя ностальгия не была сильна, нужно признать что CSS теперь вообще не big deal.
И он занимает ровно свою нишу — способ разукрасить страницу.

Безусловно CSS богат и очень мощный способ офрмлять страницу, на него сейчас можно перенести очено много всего, что по инерции часто делается на JS. Но тем не менее сегодня он перстал быть магией, потому что не нужно больше понимать как CSS работает в разных браузерах — он везде работает одинаково.

## Модульность
Все сейчас модульно. И если, например, BEМ проповедует это очень давно и выстроил некотурую инфраструктуру вокруг этого, то весь остальной мир только пришел к этому.
...
Пакетные менеджеры: bower, npm
Неограниченный источник компонент на github (js -- язк номер 1 на гитхабе)
...
Сегодня у нас на каждое сохранение работает маленький локальный вебсервер с live-reload, пересобирается js и css.
...
Вчера это был бы Node.js 0.12 + Express.js + Angular + SAAS/Stylus/Autoprefixer
Сегодня это уже Node.js 4.0.0 (с вмердженным уже io.js) + возмно hapi.js + React.js + набор PostCSS плагинов.
...
Разработчики бросают свои проекты, а у вас на них завязан проект: вы вынуждены переезжать, делать форк или брать роль мейнтейнера.
...
А вам переезжать может быть очень дорого, но это прийдется делать иначе библиотека сломается при обновлении других зависимостей или окружения.
...
Вполне нормальная история, если у вас node.js 0.10 например в продакшене то скоро множество библиотек вы не смодете использовать.

Обновиться же до новых версий — целое дело. Нужно быть уверенным что все библиотеки обновились и работают нормлаьно в новом окружении.
...
Обновление одной библиотеки часто требудет обновление других. И не всегда это омжно заметить сразу.

## Очевидный вопрос «Что делать?»
...
Но есть несколько хороших практик, которые помогут избежать головной или хотя бы начать реагировать как можно быстрее.
